...
# Tenta importar psycopg2 para o PostgreSQL (Render)
try:
    import psycopg2
...
# STRING DE CONEXÃO COM O POSTGRESQL (NUVEM RENDER)
...
def init_db_postgres():
    ...
    if not PSYCOPG2_DISPONIVEL or not DATABASE_URL:
        print("[POSTGRESQL] Biblioteca psycopg2 ou DATABASE_URL não configurada. Armazenamento na nuvem desativado.")
        return
    ...
    try:
        ...
        # Tabela 1: Lista dos Ensaios Realizados
        cur.execute("""
        ...
        # Tabela 2: Leituras individuais de cada ensaio
        cur.execute("""
            CREATE TABLE IF NOT EXISTS leituras_transdutor (
                id SERIAL PRIMARY KEY,
                ensaio_id INT REFERENCES ensaios(id) ON DELETE CASCADE,
                amostra_num INT,
                horario VARCHAR(20),
                tempo_s FLOAT,
                pressao_bar FLOAT,
                vacuo_status VARCHAR(5)
            );
        """)
        ...
        cur.close()
        conn.close()
        print("[POSTGRESQL] Tabelas inicializadas com sucesso no Render!")
    except Exception as e:
        print(f"[POSTGRESQL ERROR] Falha ao inicializar o banco de dados: {e}")
    ...
    while True:
        ...
        if not USAR_CLP_REAL and STATUS_COMUNICACAO != "CLP_REAL_CONECTADO":
            ...
            try:
                ...
                with dados_lock:
                    # Reset padrão de entradas e saídas
                    for i in range(8):
                        ESTADO_SAIDAS[f"Y{i}"] = False
                    for key in ESTADO_ENTRADAS:
                        ESTADO_ENTRADAS[key] = False

                    ESTADO_ENTRADAS["X2"] = True  # Emergência OK

                    # Posição atual no ciclo de 49 segundos
                    ...
                    # PASSO 6 (33s até 39s): Ligar Y2 (Desce eixo Z na Posição D)
                    elif 33 <= tempo_ciclo < 39:
                    ...
                    # Vinculação automática dos sensores às saídas no simulador
...
class CustomCombinedHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # 4. Rota para Salvar Ensaio no PostgreSQL (Regra de Rotação FIFO - Máx 20)
        elif self.path == "/api/salvar-ensaio":
            content_length = int(self.headers.get("Content-Length", 0))
            body = self.rfile.read(content_length).decode("utf-8")
            try:
                dados_payload = json.loads(body)
                amostras = dados_payload.get("amostras", [])
                ...
                if not amostras:
                    ...
                    self.send_header("Access-Control-Allow-Origin", "*")
                    self.end_headers()
                    self.wfile.write(json.dumps({"status": "erro", "mensagem": "Sem amostras"}).encode("utf-8"))
                    return
                ...
                if not conn:
                    ...
                    self.send_header("Access-Control-Allow-Origin", "*")
                    self.end_headers()
                    self.wfile.write(json.dumps({"status": "erro", "mensagem": "Banco de dados desativado"}).encode("utf-8"))
                    return

                cur = conn.cursor()

                # 1. Cria o registro principal do Ensaio
                cur.execute("INSERT INTO ensaios (total_amostras) VALUES (%s) RETURNING id;", (len(amostras),))
                ensaio_id = cur.fetchone()["id"]

                # 2. Insere todas as leituras de 20s
                for item in amostras:
                    cur.execute("""
                        INSERT INTO leituras_transdutor (ensaio_id, amostra_num, horario, tempo_s, pressao_bar, vacuo_status)
                        VALUES (%s, %s, %s, %s, %s, %s);
                    """, (
                        ensaio_id,
                        item.get("amostra"),
                        item.get("horario"),
                        item.get("tempo_s"),
                        item.get("pressao_bar"),
                        item.get("vacuo")
                    ))

                # 3. Lógica FIFO (Mantém apenas os últimos 20 ensaios registrados)
                cur.execute("""
                    DELETE FROM ensaios 
                    WHERE id NOT IN (
                        SELECT id FROM ensaios ORDER BY id DESC LIMIT 20
                    );
                """)
                ...
                self.send_header("Access-Control-Allow-Origin", "*")
                self.end_headers()
                self.wfile.write(json.dumps({"status": "sucesso", "ensaio_id": ensaio_id}).encode("utf-8"))
                return
            except Exception as e:
                print(f"[ERRO] Falha ao salvar ensaio no PostgreSQL: {e}")
                self.send_response(500)
                self.send_header("Content-Type", "application/json")
                self.send_header("Access-Control-Allow-Origin", "*")
                self.end_headers()
    ...
    def do_GET(self):
            ...
            with dados_lock:
                ...
                if EH_AMBIENTE_RENDER:
                    ...
                    if sem_comunicacao:
                        ...
                        # Reset padrão das saídas
                        for i in range(8):
                            ESTADO_SAIDAS[f"Y{i}"] = False
                            
                        # Reset padrão das entradas
                        for key in ESTADO_ENTRADAS:
                            ESTADO_ENTRADAS[key] = False
                        ESTADO_ENTRADAS["X2"] = True  # Emergência OK
                        
                        # -------------------------------------------------------------
                        # SEQUÊNCIA TEMPORAL SINCRONIZADA (CICLO DE 49 SEGUNDOS NO RENDER)
                        # -------------------------------------------------------------
                        tempo_ciclo = segundos % 49

                        # PASSO 1 (0s a 10s): Avança X (Y0 + Y4)
                        if 0 <= tempo_ciclo < 10:
                            ESTADO_SAIDAS["Y0"] = True
                            ESTADO_SAIDAS["Y4"] = True

                        # PASSO 2 (10s a 13s): Desce Z (Y2)
                        elif 10 <= tempo_ciclo < 13:
                            ESTADO_SAIDAS["Y2"] = True

                        # PASSO 3 (13s a 23s): Liga Vácuo (Y3)
                        elif 13 <= tempo_ciclo < 23:
                            ESTADO_SAIDAS["Y3"] = True

                        # PASSO 5 (23s a 33s): Transporta até a Posição D (Y0 + Y1 + Y3)
                        elif 23 <= tempo_ciclo < 33:
                            ESTADO_SAIDAS["Y0"] = True
                            ESTADO_SAIDAS["Y1"] = True
                            ESTADO_SAIDAS["Y3"] = True

                        # PASSO 6 (33s a 39s): Desce Z na Posição D (Y1 + Y2 + Y3)
                        elif 33 <= tempo_ciclo < 39:
                            ESTADO_SAIDAS["Y1"] = True
                            ESTADO_SAIDAS["Y2"] = True
                            ESTADO_SAIDAS["Y3"] = True

                        # PASSO 7 (39s a 42s): Desliga Vácuo para soltar a peça (Y1 + Y2)
                        elif 39 <= tempo_ciclo < 42:
                            ESTADO_SAIDAS["Y1"] = True
                            ESTADO_SAIDAS["Y2"] = True

                        # PASSO 8 (42s a 45s): Sobe Z sem a peça (Y1)
                        elif 42 <= tempo_ciclo < 45:
                            ESTADO_SAIDAS["Y1"] = True

                        # PASSO 9 (45s a 49s): Retorna Y para a origem (Todas as saídas False)
                        elif 45 <= tempo_ciclo < 49:
                            pass

                        # Vinculação automática dos sensores às saídas no simulador da nuvem
                        ESTADO_ENTRADAS["X23"] = ESTADO_SAIDAS.get("Y1", False)
                        ESTADO_ENTRADAS["X25"] = ESTADO_SAIDAS.get("Y3", False)
        ...
        # 5. Rota GET: Listar os 20 últimos ensaios para preencher a Tabela no Modal
        elif path == "/api/historico/ensaios":
            try:
                conn = get_db_connection()
                if not conn:
                    self.send_response(500)
                ...
                cur = conn.cursor()
                cur.execute("""
                    SELECT id, TO_CHAR(data_hora, 'DD/MM/YYYY HH24:MI:SS') as data_formatada, total_amostras 
                    FROM ensaios 
                ...
                self.send_header("Access-Control-Allow-Origin", "*")
                self.end_headers()
                self.wfile.write(json.dumps(ensaios).encode("utf-8"))
                return
            except Exception as e:
                print(f"[ERRO] Falha ao listar ensaios: {e}")
                self.send_response(500)
                self.end_headers()

        # 6. Rota GET: Detalhes de um ensaio específico (Ex: /api/historico/ensaio/15)
        elif path.startswith("/api/historico/ensaio/"):
            try:
                ensaio_id = int(path.split("/")[-1])
                conn = get_db_connection()
                if not conn:
                    self.send_response(500)
                    self.end_headers()
                ...
                cur = conn.cursor()
                cur.execute("""
                    SELECT amostra_num, horario, tempo_s, pressao_bar, vacuo_status 
                    FROM leituras_transdutor 
                    WHERE ensaio_id = %s 
                    ORDER BY amostra_num ASC;
                """, (ensaio_id,))
                ...
                self.send_header("Access-Control-Allow-Origin", "*")
                self.end_headers()
                self.wfile.write(json.dumps(leituras).encode("utf-8"))
                return
            except Exception as e:
                print(f"[ERRO] Falha ao obter leituras do ensaio: {e}")
                self.send_response(500)
                self.end_headers()

        # 7. Servir arquivos estáticos (STL, WebP, JPG, PNG e outros da pasta assets)
...
if __name__ == "__main__":
    ...
    # Inicializa as tabelas no PostgreSQL do Render se a URL estiver disponível
    init_db_postgres()
